...
import { setSearchItems, setUserData ,setCurrentAddress} from '../redux/userSlice';
...
function Nav() {
  ...
  try {
    const result = await axios.get(
      `${serverUrl}/api/item/searchitems?query=${searchQuery}&city=${currentCity}`,
      { withCredentials: true }
    );

    // ✅ Only update if query still same
    if (searchQuery === query && query.trim() !== "") {
  ...
  if (query) {
    ...
    return (
        <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible'>
            ...
            <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d]'>GroDel</h1>
