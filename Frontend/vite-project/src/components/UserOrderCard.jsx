...
function UserOrderCard({ data }) {
  ...
  const handleRating = async (itemId, rating) => {
    const isAlreadySelected = selectedRating[itemId] === rating
    const newRating = isAlreadySelected ? 0 : rating  // toggle off if same star clicked

    try {
        const result = await axios.post(`${serverUrl}/api/item/rating`, { itemId, rating: newRating }, { withCredentials: true })
        setSelectedRating(prev => ({
            ...prev, [itemId]: newRating
        }))
    } catch (error) {
        console.log(error)
    }
