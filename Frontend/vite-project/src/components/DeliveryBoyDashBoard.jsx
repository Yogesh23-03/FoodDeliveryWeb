...
useEffect(()=>{
  if (!socket) return   // ✅ FIX 1: prevent null crash

  socket.on('newAssignment',(data)=>{
    setAvailableAssignments(prev => ([...(prev || []), data])) // ✅ FIX 2
  })
...
const cancelOrder = async () => {
  // Use the assignment ID from the currentOrder object
  const assignmentId = currentOrder?.shopOrder?.assignment; 
  
  if (!assignmentId) {
    alert("Assignment reference not found.");
    return;
  }

  const confirmCancel = window.confirm("This will return the order to the public pool. Are you sure?");
...
{!showOtpBox ? (
      <div className="flex flex-col gap-2 mt-4">
        <button 
          className='w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-xl shadow-md hover:bg-green-600 active:scale-95 transition-all' 
          onClick={sendOtp} 
          disabled={loading}
        >
        ...
        {/* NEW CANCEL BUTTON */}
        <button 
          className='w-full bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-xl hover:bg-red-100 hover:text-red-600 transition-all'
          onClick={cancelOrder}
          disabled={loading}
        >
          Cancel Delivery
